...
如果脚本需要在生产端预编译，然后把预编译结果通过 JSON 分发到其他机器执行，可以使用可序列化预编译缓存。QLExpress 只提供 JavaBean DTO，不引入 JSON 库；业务方可以自行选择 Jackson、Fastjson2、Gson 等库序列化 `SerializableParseCache`。
...
高频执行时可以先在消费端加载为 `LoadedParseCache`，避免每次执行都重新绑定类和运算符：
...
* 生产端和消费端的 `Express4Runner` 环境必须等价，包括默认 import、类加载、用户函数、自定义运算符和安全策略。
* `LoadedParseCache` 是和创建它的 `Express4Runner` 绑定的对象，不保证可跨 Runner 复用。
* 包含函数定义的远程 DTO 也可以通过 `addFunctionsDefinedInScript(SerializableParseCache, ...)` 或 `addFunctionsDefinedInScript(LoadedParseCache, ...)` 注册到消费端 Runner。
* 可序列化模型有独立的 `modelVersion`，不会跟随 QLExpress 发布版本自动变化，相同 `modelVersion` 可以跨 QLExpress 发布版本执行。
* 远程预编译脚本不会自动写入或读取 Runner 内部的 `compileCache`，应用需要自行管理 DTO 或 `LoadedParseCache` 的生命周期。
...
3. 执行单元测试：在开发完成代码之后，先在本地全量执行 `mvn test`，确保代码质量。
