...
QLExpress4 作为 QLExpress 的最新演进版本，采用手写的递归下降解析器重写了解析引擎，移除了对 ANTLR4 的依赖，进一步减小了 JAR 包体积，提升了启动性能。新版本将原先的优点进一步发扬光大，新增了大量特色功能，彻底拥抱函数式编程，在性能和表达能力上都进行了进一步增强。
...
QLExpress 使用手写的递归下降解析器作为解析引擎。编译过程中会产生缓存以提升后续脚本解析的性能。这个缓存会占用一定的内存空间。

在某些内存敏感的场景下，可以通过调用 `clearCompileCache` 方法来清除编译缓存，释放内存。

> **重要警告**: 清除编译缓存会导致编译性能大幅下降，正常情况下不建议使用此方法。
