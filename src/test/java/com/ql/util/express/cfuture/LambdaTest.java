...
public class LambdaTest {
    ...
    public void lambdaOperatorTest() throws Exception {
        ...
        Object res = runner.execute("l map (item, index)->item+index", context, null, true, true);
        Assert.assertEquals(Arrays.asList("a0", "b1", "c2"), res);

        Object res1 = runner.execute("l map (item,index)->item+index", context, null, true, true);
        Assert.assertEquals(Arrays.asList("a0", "b1", "c2"), res1);
    }

    @Test
    public void streamTest() throws Exception {
        List<Integer> a = Stream.of(1, 2, 3).filter(item -> item > 1).map(item -> item + 1)
            .collect(Collectors.toList());
        a.stream().forEach(System.out::println);
        Assert.assertEquals(Arrays.asList(3, 4), a);

        String expr = ""
            + "a = NewList(1,2,3).stream()"
            + "    .filter(item -> item > 1)"
            + "    .map(item->item+1)"
            + "    .collect(Collectors.toList());"
            ...
            + "return a;";
        ExpressRunner runner = new ExpressRunner(false, true);
        Object res = runner.execute(expr, new DefaultContext<>(), null, false, false);
