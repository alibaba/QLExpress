 ...
 *
 * Bug: TryCatchInstruction.execute() must propagate real loop break/continue,
 * but must not treat normal block-expression CONTINUE results as loop continue.
 */
public class TryCatchBreakContinueTest {
    
    private static final InitOptions INIT_OPTIONS =
        InitOptions.builder().securityStrategy(QLSecurityStrategy.open()).build();
    
    private static final QLOptions LOOP_TEST_OPTIONS = QLOptions.builder().timeoutMillis(2000).build();
    ...
    public void breakInsideTryShouldExitLoop() {
        ...
        String script =
            "result = 0;\n" + "for(int i = 0; i < 10; i++) {\n" + "  try {\n" + "    if (i == 3) { break; }\n"
                + "    result = result + 1;\n" + "  } catch(Exception e) {}\n" + "}\n" + "result;";
        Object result = runner.execute(script, new HashMap<>(), LOOP_TEST_OPTIONS).getResult();
    ...
    @Test
    public void normalTryExpressionInsideWhileShouldNotSkipFollowingStatement() {
        Express4Runner runner = new Express4Runner(INIT_OPTIONS);
    ...
    @Test
    public void continueInsideWhileTryShouldSkipRestOfBody() {
        Express4Runner runner = new Express4Runner(INIT_OPTIONS);
        String script = "i = 0; result = 0; tail = 0;\n" + "while(i < 5) {\n" + "  i = i + 1;\n" + "  try {\n"
            + "    if (i == 3) { continue; }\n" + "    result = result + i;\n" + "  } catch(Exception e) {}\n"
            + "  tail = tail + 10;\n" + "}\n" + "result * 100 + tail;";
        Object result = runner.execute(script, new HashMap<>(), LOOP_TEST_OPTIONS).getResult();
        Assert.assertEquals(1240, result);
    }
