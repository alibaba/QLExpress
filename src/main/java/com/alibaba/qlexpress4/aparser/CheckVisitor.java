...
public class CheckVisitor extends QLParserBaseVisitor<Void> {
     ...
     */
    private void checkOperator(String operatorString, Token token) throws QLSyntaxException {
        switch (strategy) {
            case ALLOW_ALL:
                // No restriction
                return;
            ...
            case WHITELIST:
                if (operatorStrings == null || !operatorStrings.contains(operatorString)) {
                    String reason = String.format("Script uses disallowed operator: %s. Allowed operators: %s",
                        operatorString, operatorStrings);
                    throw QLException.reportScannerErr(script,
                        token.getStartIndex(),
                        token.getLine(),
                        token.getCharPositionInLine(),
                        operatorString, "OPERATOR_NOT_ALLOWED", reason);
                }
                break;
            ...
            case BLACKLIST:
                if (operatorStrings != null && operatorStrings.contains(operatorString)) {
                    ...
                    throw QLException.reportScannerErr(script,
                        ...
                        token.getLine(),
                        token.getCharPositionInLine(),
                        operatorString, "OPERATOR_FORBIDDEN", reason);
                }
                break;
        }
    }
