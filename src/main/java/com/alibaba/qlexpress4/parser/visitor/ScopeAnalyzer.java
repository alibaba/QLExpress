...
public class ScopeAnalyzer implements ASTVisitor<Void, ScopeAnalyzer.Context> {
    ...
    public static class ShadowedVariable {
        ...
        public String toString() {
            return String.format("'%s' declared at %d:%d shadows declaration at %d:%d",
                    name, shadowing.getLine(), shadowing.getColumn(),
                    shadowed.getLine(), shadowed.getColumn());
        }
    }
     ...
     */
    public Context analyze(ASTNode node) throws Exception {
        if (node == null) {
            return new Context();
        }
    ...
    public Void visit(BlockNode node, Context context) throws Exception {
        ...
        Scope blockScope = new Scope(ScopeType.BLOCK, context.getCurrentScope(),
    ...
    public Void visit(ForNode node, Context context) throws Exception {
        ...
        Scope forScope = new Scope(ScopeType.FOR_LOOP, context.getCurrentScope(),
                node.getLine(), node.getColumn());
    ...
    private Void visitSwitchCase(SwitchCaseNode caseNode, Context context) throws Exception {
    ...
    private Void visitCatchClause(CatchClauseNode catchClause, Context context) throws Exception {
        // Create a new catch clause scope
        Scope catchScope = new Scope(ScopeType.CATCH_CLAUSE, context.getCurrentScope(),
        ...
        if (exceptionVar != null && !exceptionVar.isEmpty()) {
            ...
            if (existing != null && existing != context.getCurrentScope().getVariables().get(exceptionVar)) {
                ...
                VariableInfo newVar = new VariableInfo(exceptionVar, typeName,
                        catchClause.getBody().getLine(), catchClause.getBody().getColumn(),
                        context.getCurrentScope());
                context.addShadowedVariable(new ShadowedVariable(exceptionVar, newVar, existing));
            }

            context.getCurrentScope().addVariable(exceptionVar,
                    new VariableInfo(exceptionVar, typeName,
                            catchClause.getBody().getLine(), catchClause.getBody().getColumn(),
                            context.getCurrentScope()));
        }
    ...
    public Void visit(VariableDeclarationNode node, Context context) throws Exception {
        ...
        if (existing != null && !context.getCurrentScope().containsVariable(varName)) {
            ...
            VariableInfo newVar = new VariableInfo(varName, typeName, node.getLine(), node.getColumn(),
                    context.getCurrentScope());
            context.addShadowedVariable(new ShadowedVariable(varName, newVar, existing));
        }

        context.getCurrentScope().addVariable(varName,
                new VariableInfo(varName, typeName, node.getLine(), node.getColumn(),
                        context.getCurrentScope()));
    ...
    public Void visit(FunctionDefinitionNode node, Context context) throws Exception {
        ...
        Scope functionScope = new Scope(ScopeType.FUNCTION, context.getCurrentScope(),
                node.getLine(), node.getColumn());
    ...
    public Void visit(LambdaNode node, Context context) throws Exception {
        ...
        Scope lambdaScope = new Scope(ScopeType.LAMBDA, context.getCurrentScope(),
                node.getLine(), node.getColumn());
        ...
        for (ParameterNode param : node.getParameters()) {
            ...
            if (existing != null && !context.getCurrentScope().containsVariable(paramName)) {
                ...
                VariableInfo newParam = new VariableInfo(paramName, typeName,
                        node.getLine(), node.getColumn(), context.getCurrentScope());
                context.addShadowedVariable(new ShadowedVariable(paramName, newParam, existing));
            }

            context.getCurrentScope().addVariable(paramName,
                    new VariableInfo(paramName, typeName, node.getLine(), node.getColumn(),
                            context.getCurrentScope()));
        }
