...
public class VariableDetector implements ASTVisitor<Void, VariableDetector.Context> {
     ...
     */
    public Context detect(ASTNode node) throws Exception {
        if (node == null) {
            return new Context();
        }
    ...
    public Void visit(TryCatchNode node, Context context) throws Exception {
        ...
        for (CatchClauseNode catchClause : node.getCatchClauses()) {
            ...
            if (exceptionVar != null && !exceptionVar.isEmpty()) {
                ...
                context.addVariableDeclaration(new VariableDeclaration(
                        typeName, exceptionVar, catchClause.getBody().getLine(), catchClause.getBody().getColumn()));
            }
    ...
    public Void visit(VariableDeclarationNode node, Context context) throws Exception {
        ...
        context.addVariableDeclaration(new VariableDeclaration(
                node.getTypeName(), node.getVariableName(), node.getLine(), node.getColumn()));
    ...
    public Void visit(AssignmentNode node, Context context) throws Exception {
        ...
        if (node.getTarget() instanceof IdentifierNode) {
            ...
            context.addVariableWrite(new VariableAccess(
                    VariableAccessType.WRITE, target.getName(), node.getLine(), node.getColumn()));
        } else {
    ...
    public Void visit(IdentifierNode node, Context context) throws Exception {
        ...
        context.addVariableRead(new VariableAccess(
                VariableAccessType.READ, node.getName(), node.getLine(), node.getColumn()));
        return null;
    }
    ...
    public Void visit(BinaryOpNode node, Context context) throws Exception {
        ...
        boolean isAssignment = operator.equals("=") ||
                             operator.equals("+=") ||
                             operator.equals("-=") ||
                             operator.equals("*=") ||
                             operator.equals("/=") ||
        ...
        if (isAssignment && node.getLeft() instanceof IdentifierNode) {
            ...
            if (!operator.equals("=")) {
                context.addVariableRead(new VariableAccess(
                        VariableAccessType.READ, target.getName(), node.getLine(), node.getColumn()));
            }
        } else {
    ...
    public Void visit(LambdaNode node, Context context) throws Exception {
        ...
        for (ParameterNode param : node.getParameters()) {
            context.addVariableDeclaration(new VariableDeclaration(
                    param.getTypeName(), param.getParameterName(), node.getLine(), node.getColumn()));
        }
