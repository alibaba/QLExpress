...
/**
 * OutVarAttrDetector detects variable attribute access paths (like a.b.c) that are read but not declared.
 * <p>
 * This visitor tracks field access chains and determines which variables are external.
 * Field access paths like "a.b.c" are tracked as a complete path.
 *
...
public class OutVarAttrDetector implements ASTVisitor<Void, OutVarAttrDetector.Context> {
     ...
     * <p>
     * Tracks declared variables and collects out variable attributes.
     */
    public static class Context {
         ...
         *
         * @param path the attribute path (e.g., ["a", "b", "c"] for a.b.c)
         */
        public void readVariablePath(List<String> path) {
            if (path.isEmpty()) {
                return;
            }
            String rootVar = path.get(0);
            if (!isDeclared(rootVar)) {
         ...
         *
         * @return the set of out variable attribute paths
         */
    ...
    public Void visit(AssignmentNode node, Context context)
        ...
        if (node.getTarget() instanceof IdentifierNode) {
            ...
            if (!operator.equals("=")) {
                context.readVariablePath(Collections.singletonList(target.getName()));
            }
            // Then declare it (assignment is a declaration)
            context.declareVariable(target.getName());
        }
        else {
            // For complex targets, track the path and then visit
    ...
    public Void visit(BinaryOpNode node, Context context)
        ...
        if (isAssignment && node.getLeft() instanceof IdentifierNode) {
            ...
            // For non-assignment expressions, extract path from left side if possible
    ...
    public Void visit(FieldAccessNode node, Context context)
        throws Exception {
        // Extract the full path from field access
    ...
    public Void visit(MethodCallNode node, Context context)
        throws Exception {
        // Visit the target - it might be a field access chain
        visitNode(node.getTarget(), context);
     ...
     * For example:
     * - IdentifierNode "a" -> ["a"]
     * - FieldAccessNode(a.b) -> ["a", "b"]
     * - FieldAccessNode(FieldAccessNode(a.b).c) -> ["a", "b", "c"]
     *
     * @param node the expression node
     * @return the path as a list of strings, or empty list if not a simple path
     */
    private List<String> extractPath(Node node) {
        if (node == null) {
        ...
        if (node instanceof FieldAccessNode) {
            ...
            List<String> targetPath = extractPath(fieldAccess.getTarget());
            if (!targetPath.isEmpty()) {
                path.addAll(targetPath);
                path.add(fieldAccess.getFieldName());
                return path;
            }
        }
     ...
     * @return the set of out variable attribute paths
     */
    public Set<List<String>> detect(ASTNode node)
        throws Exception {
        if (node == null) {
