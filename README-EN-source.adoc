...
QLExpress4, as the latest evolution of QLExpress, rewrote the parsing engine using a hand-written recursive descent parser, removing the dependency on ANTLR4, further reducing JAR size, and improving startup performance. The new version further enhances the original advantages and fully embraces functional programming, with further improvements in both performance and expressive power.
...
=== Clearing Compilation Cache

QLExpress uses a hand-written recursive descent parser as its parsing engine. During compilation, a cache is generated to accelerate subsequent script parsing. This cache consumes a certain amount of memory.

In some memory-sensitive scenarios, you can call the `clearCompileCache` method to clear the compilation cache and release memory.

> **IMPORTANT WARNING**: Clearing the compilation cache will cause a significant compilation performance degradation. It is NOT recommended for normal use cases.
